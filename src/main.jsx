...
import { Download, Plus, Trash2, User, Utensils, Target, Scale, Save, FileText, Database, BarChart3 } from 'lucide-react';
import './style.css';
...
function App(){
 ...
 return <div className="app">
  <aside><div className="brand"><b>Sclass</b><span>Fitness Coach App</span></div><button onClick={()=>window.print()}><FileText/> Export / Print PDF</button><button onClick={exportJson}><Download/> Save Client JSON</button><label className="import"><Save/> Import JSON<input type="file" accept=".json" onChange={importJson}/></label><button onClick={()=>{if(confirm('Reset this dashboard?'))setState(defaultState)}}><Trash2/> Reset</button></aside>
  <main>
